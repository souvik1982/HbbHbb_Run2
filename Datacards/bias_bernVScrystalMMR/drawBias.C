void drawBias () {
	...
	biasG2[dd]->SetMarkerStyle(20);
        ...
        for (int i=0; i!=nPoints; ++i) {
 	    ...
 	    int mass[nPoints] ={450,550,650,750,900};
	    ...
	    hists[dd][i]->SetLineColor(kBlack);
            ...
            //else
            tree_fit_sb[dd][i]->Draw("(mu-2.0)/muHiErr>>"+name,"fit_status >= 0","pe");
	...
	//    if(mass[i]==350) biasG[dd]->SetPoint(i,mass[i],hists[dd][i]->GetFunction("gaus")->GetParameter(1)/2.);
            ...
            biasG2[dd]->SetPoint(i,mass[i],hists[dd][i]->GetMean());//,quantile); // //GetMean
