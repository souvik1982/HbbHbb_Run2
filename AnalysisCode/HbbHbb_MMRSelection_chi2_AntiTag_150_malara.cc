...
#include <iostream>
#include <vector>
#include "/scratch/malara/WorkingArea/HbbHbb_Run2/AnalysisCode/PDFs/BtagCalibrationStandalone.cc"
...
void HbbHbb_MMRSelection_chi2_AntiTag_150_malara(std::string type,std::string source_dir, std::string dest_dir, std::string sample )
{
  ...
  std::string histfilename=dest_dir+"/Histograms_MMR_chi2_"+sample+".root";
            ...
            for (unsigned int l=0; l<jetIndex_CentralpT40btag_CMVAOrder->size(); ++l)
            {
              ...
              if (l_jetIndex!=k_jetIndex && l_jetIndex!=j_jetIndex && jet3_p4.Pt()>jet_pT_cut1)
              {
                for (unsigned int m=0; m<jetIndex_CentralpT40_CSVOrder->size(); ++m)
                {
                  ...
                  if (m_jetIndex!=l_jetIndex && m_jetIndex!=k_jetIndex && m_jetIndex!=j_jetIndex && jet_btagCSV[m_jetIndex]<0.185 && jet4_p4.Pt()>jet_pT_cut1)
                  {
