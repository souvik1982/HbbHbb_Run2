...
#include </scratch/malara/WorkingArea/release/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/interface/RooSpline1D.h>

RooSpline1D* create_spline(std::string file_name, std::string histo_name, std::string spline_name, int rebin, int range_lo, int range_hi, RooRealVar* x){

    TFile *f=new TFile((file_name).c_str());
    TH1F *h=(TH1F*)f->Get((histo_name).c_str());
    h->Rebin(rebin);    h->Scale(1./h->Integral());

    int bin_min = h->FindBin(range_lo);
    int bin_max = h->FindBin(range_hi);
    std::vector<double> v_x, v_y;
h->GetXaxis()->SetRangeUser(range_lo,range_hi);

    for (int i = bin_min; i<= bin_max; i++) {	v_y.push_back(h->GetBinContent(i));	v_x.push_back(h->GetBinCenter(i)); }

    RooSpline1D* spline= new RooSpline1D((spline_name).c_str(), (spline_name).c_str(), *x, bin_max-bin_min+1, &v_x[0], &v_y[0], "CSPLINE");
    return spline;
}
                                                 ...
                                                 std::string dest_dir="/scratch/malara/WorkingArea/IO_file/output_file/DeepCSV/MMR/fit")
{
    ...
    h_SR_AntiTag->Divide(h_SB_AntiTag);
    h_SR_AntiTag->Multiply(h_SB);
    ...
    TCanvas *c_landau=new TCanvas("c_landau", "c_landau", 800, 800);
    frame_landau->Draw();

    TPaveText *pave_landau = new TPaveText(0.90,0.7,0.67,0.9,"NDC");
    ...
    w_spline->import(*spline_SB_AntiTag);
    w_spline->import(*spline_SB);
}
