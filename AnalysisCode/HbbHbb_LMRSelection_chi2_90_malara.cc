...
double mean_H1_mass_=90;//120;
double sigma_H1_mass_=20;//25;
double mean_H2_mass_=90;//115;
...
void HbbHbb_LMRSelection_chi2_90_malara(std::string type,std::string source_dir, std::string dest_dir, std::string sample, int signal_mass = 300, bool reg = false  )
{
              ...
              if (l_jetIndex!=k_jetIndex && l_jetIndex!=j_jetIndex && jet3_p4.Pt()>jet_pT_cut1)
              {
                for (unsigned int m=0; m<jetIndex_CentralpT40btag_CMVAOrder->size(); ++m)
                {
                  ...
                  if (m_jetIndex!=l_jetIndex && m_jetIndex!=k_jetIndex && m_jetIndex!=j_jetIndex && jet4_p4.Pt()>jet_pT_cut1)
                  {
                    ...
                    if (chi2<chi2_old && ((50.<mH1 && mH1<130.) && (50.<mH2 && mH2<130.)))
                    {
                      H1jet1_i=j_jetIndex;
                      H1jet2_i=k_jetIndex;
                      H2jet1_i=l_jetIndex;
