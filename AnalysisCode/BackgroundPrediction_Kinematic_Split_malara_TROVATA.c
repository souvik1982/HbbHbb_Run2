...
void BackgroundPrediction_Kinematic_Split_malara(double plot_lo, double plot_hi, double rebin,
                                                ...
                                                std::string dest_dir="/scratch/malara/WorkingArea/IO_file/output_file/DeepCSV/LMR/fit")
{
    ...
    RooRealVar *x = new RooRealVar("x", "m_{X} (GeV)", 240, 650);
...
//new TCanvas; data_plot_test_3->Draw();
    ...
    RooFitResult *r_bg_sum_gauss=bg_sum_gauss->fitTo(pred, RooFit::Range(275, 450), RooFit::Save());
    ...
    RooRealVar ml("ml","mean landau",2.54e+02,2.5e+02,2.8e+02) ;
    ...
    RooAddPdf* bg_landau_pol = new RooAddPdf("bg_landau_pol", "bg_landau_pol", RooArgList(landau, bg_pol3), bg_land_pol_add); 

    RooPlot *data_plot7=x->frame();
    int fit_pol_land_0=253;
    int fit_pol_land_1=552;
    ...
    RooDataHist data_obs_falling3_1(Form("data_obs_falling3_%d", crystal_3), "Data", RooArgList(*x_13), h_mX_SR_fakeData);
