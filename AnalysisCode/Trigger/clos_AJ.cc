...
void clos_AJ()
{
  ...
  tree->SetBranchAddress("nJet", &(nJet)); tree->SetBranchStatus("nJet", 1);
    ...
    Long64_t nEvents = 10000000;//tree->GetEntries(); 
...
void FillHistos( TH1 * histos[4], float variable, float sumpt, float pt2, float pt4, float CSV3, int pass_trigger1, int pass_trigger2 )
{
    ...
    if( pass_trigger1 ){ weight += TurnOnDouble( sumpt, pt2, pt4, CSV3 ); weightUp += TurnOnDoubleUp( sumpt, pt2, pt4, CSV3 ); weightDown += TurnOnDoubleDown( sumpt, pt2, pt4, CSV3 ); }
    if( pass_trigger2 ){ weight += TurnOnQuad( sumpt, pt2, pt4, CSV3 ); weightUp += TurnOnQuadUp( sumpt, pt2, pt4, CSV3 ); weightDown += TurnOnQuadDown( sumpt, pt2, pt4, CSV3 ); }
    if( pass_trigger1&&pass_trigger2 ){ weight-=TurnOnDouble( sumpt, pt2, pt4, CSV3 )*TurnOnQuad( sumpt, pt2, pt4, CSV3 ); weightUp-=TurnOnDoubleUp( sumpt, pt2, pt4, CSV3 )*TurnOnQuadUp( sumpt, pt2, pt4, CSV3 ); weightDown-=TurnOnDoubleDown( sumpt, pt2, pt4, CSV3 )*TurnOnQuadDown( sumpt, pt2, pt4, CSV3 ); }


    
    if( pass_trigger1 || pass_trigger2 ){ histos[0]->Fill( variable, 1 ); } 
