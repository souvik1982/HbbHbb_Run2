...
void TE_presel()
{
  ...
  // TChain* tree = (TChain*) _file0->Get("tree");
    ...
    Float_t  jet_pt[100], Jet_eta[100], Jet_btagCSV[100];
 ...
 /* outtree->Branch("Jet_pt",&Jet_pt,"Jet_pt/F");
  ...
  for (Int_t i=0; i<nEvents; ++i)
  {
      ...
      Int_t counter = 0;
      Int_t GoodJets[100];
      for( Int_t j=0; j<nJet; ++j ) if( abs(Jet_eta[j])<2.4 && Jet_puId[j]>=4 ){ GoodJets[counter]=j; CSVsorted[counter]=Jet_btagCSV[j]; counter++;  }
