...
//    TFile *_file0 = TFile::Open("SingleElectronVBF.root");
 ...
 tree->Draw("Jet_pt[2]  >> h(25,0,200)"," DoubleTriggerWeight(Jet_pt[0], Jet_pt[1], Jet_pt[2], Jet_pt[3], min(CSV[2],1-1.e-7))*(HLT_BIT_HLT_Ele23_WPLoose_Gsf_v && Alt$(Jet_pt[3],0)>30 && abs(Jet_eta[0])<2.4 && abs(Jet_eta[1])<2.4 && abs(Jet_eta[2])<2.4 && abs(Jet_eta[3])<2.4)","");
    h = (TProfile*) gDirectory->Get("h");h->SetLineColor(kBlack);h->SetMaximum(2.0*h->GetMaximum());
    tree->Draw("Jet_pt[2]"," 1.0001*(HLT_BIT_HLT_DoubleJet90_Double30_TripleBTagCSV0p67_v && (HLT_BIT_HLT_Ele23_WPLoose_Gsf_v && Alt$(Jet_pt[3],0)>30 && abs(Jet_eta[0])<2.4 && abs(Jet_eta[1])<2.4 && abs(Jet_eta[2])<2.4 && abs(Jet_eta[3])<2.4))","same");

    c1->SaveAs("check_full_pt2_single.png");
    c1->SaveAs("check_full_pt2_single.C");
